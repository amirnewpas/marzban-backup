...
function extract_password() {
    if [[ -f "$ENV_FILE" ]]; then
        ...
        [[ $? -eq 0 ]] || { echo "خطا در استخراج رمز عبور"; exit 1; }
    else
        echo "$ENV_FILE پیدا نشد!"; exit 1
    fi
}
...
function backup_and_send() {
    ...
    [[ -n "$MYSQL_ROOT_PASSWORD" ]] || { echo "فایل رمز عبور خالی است."; exit 1; }
    [[ -n "$TELEGRAM_BOT_TOKEN" && -n "$TELEGRAM_CHAT_ID" ]] || { echo "توکن بات یا شناسه چت تنظیم نشده است"; exit 1; }

    mkdir -p "$DB_DIR" "$OPT_DIR" "$VARLIB_DIR"

    show_progress 10 "استخراج رمز عبور"; sleep 1
    show_progress 25 "شروع پشتیبان گیری..."; sleep 1
    ...
    show_progress 35 "خروجی گرفتن از دیتابیس..."; sleep 1
    ...
    show_progress 60 "پشتیبان گیری از /opt/marzban..."; sleep 1
    ...
    show_progress 75 "پشتیبان گیری از /var/lib/marzban..."; sleep 1
...
"
    ...
    if echo "$response" | grep -q "\"ok\":true"; then
        show_progress 100 "✅ ارسال به تلگرام با موفقیت انجام شد"
        echo ""
        rm -f "$BASE_DIR/$FINAL_ARCHIVE"
    else
        echo -e "\n❌ ارسال به تلگرام موفق نبود."
...
function change_cron_only() {
    ...
    echo "✅ کران جاب هر $INTERVAL ساعت تنظیم شد"
}
...
function run_backup() {
    [[ -f /root/.telegram_bot_token ]] && TELEGRAM_BOT_TOKEN=$(cat /root/.telegram_bot_token) || { echo "❌ توکن بات یافت نشد."; return 1; }
    [[ -f /root/.telegram_chat_id ]] && TELEGRAM_CHAT_ID=$(cat /root/.telegram_chat_id) || { echo "❌ شناسه چت یافت نشد."; return 1; }
...
function settings_menu() {
    clear
    echo "=== تنظیمات ==="
    echo "Bot Token: $(cat /root/.telegram_bot_token 2>/dev/null || echo 'تنظیم نشده')"
    echo "Chat ID: $(cat /root/.telegram_chat_id 2>/dev/null || echo 'تنظیم نشده')"
    echo "Cron job:"
    crontab -l 2>/dev/null | grep "$BACKUP_SCRIPT_PATH" | while read -r line; do
        echo "-> $line"
    done || echo "کران جاب پیدا نشد"
    echo "------------------"
    echo "1) تغییر Bot Token"
    echo "2) تغییر Chat ID"
    echo "3) تغییر فاصله اجرای کران جاب"
    echo "4) بازگشت"
    read -rp "انتخاب کنید: " input
    case $input in
        1) echo "توکن جدید را وارد کنید:"; read -r token; echo "$token" > /root/.telegram_bot_token ;;
        2) echo "شناسه چت جدید را وارد کنید:"; read -r id; echo "$id" > /root/.telegram_chat_id ;;
        3) change_cron_only ;;
        *) ;;
    esac
    read -rp "برای ادامه کلید بزنید..."
}

function show_menu() {
    clear
    echo "=============================="
    echo " منوی مدیریت پشتیبان گیری مرزبان"
    echo "=============================="
    LAST_BACKUP="پشتیبان گرفته نشده"
    if [[ -f /root/.last_backup_time ]]; then
        LAST_BACKUP=$(cat /root/.last_backup_time)
    fi
    echo "آخرین پشتیبان: $LAST_BACKUP"
    echo "=============================="
    echo "1) نصب و تنظیم تلگرام بات و کران جاب"
    echo "2) اجرای پشتیبان گیری و ارسال به تلگرام"
    echo "3) تنظیمات"
    echo "4) خروج"
    echo "=============================="
    read -rp "یک گزینه انتخاب کنید: " option
    case $option in
        1) setup_cron ;;
        2) run_backup ;;
        3) settings_menu ;;
